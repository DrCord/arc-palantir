<template>
  <div class="fleet-vehicle-remove-container">
    ...
    <div v-if="availableVins.length">
      <vue-form :state="formstate" @submit.prevent>
        ...
        >Remove Vehicle</v-btn>
      </vue-form>
    </div>
    <div v-else>
      <h4 v-if="loading">Loading Data...</h4>
      <h4 v-else>No vehicles available to remove from fleet</h4>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions({
      fleetVehicleRemoveAction: FLEET_VEHICLE_REMOVE,
    ...
    async vehicleFleetRemove () {
      ...
      await this.fleetVehicleRemoveAction({ componentSelf, payload })
...
#app {
  .context-menu {
    .v-list-item {
      .fleet-vehicle-remove-container {
        width: 100%;
        h3 {
          margin-bottom: 20px;
        }
